...
import { Search, Filter, MapPin, Grid, Map, PlusCircle } from 'lucide-react';
...
export const LostItemsPage: React.FC<LostItemsPageProps> = ({
  ...
  const lostItems = items.filter(i => i.type === 'lost' && !i.isFraudulent);
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 pb-2 border-b border-slate-200">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">Lost Items Registry</h1>
          </div>
          <p className="text-xs text-slate-500 mt-1">
            Browse all actively reported lost belongings across public venues and transit systems.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs px-4 py-2.5 shadow-xs transition-colors"
          >
      ...
      {viewMode === 'grid' ? (
        filteredItems.length === 0 ? (
          <div className="py-16 text-center rounded-3xl bg-white border border-slate-200 p-8">
            ...
            <h3 className="font-bold text-slate-800 text-sm">No lost items match your criteria</h3>
            <p className="text-xs text-slate-500 mt-1">Try relaxing filters or check the Found Items registry.</p>
          </div>
        ) : (
